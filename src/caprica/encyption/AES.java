package caprica.encyption;

import caprica.system.Output;

public class AES {

    String message;
    
    int rounds = 10;
 
    private int[][] toMatrix( int[] state ){
        
        int[][] matrix = new int[ 4 ][ 4 ];
        
        for ( int x = 0 ; x < 4 ; x++ ){
            
            for ( int y = 0 ; y < 4 ; y++ ){
                
                matrix[ x ][ y ] = state[ x + ( 4 * y ) ];
                
            }
            
        }
        
        return matrix;
        
    }
    
    private int[] subBytes( int[] state ){
        
        for ( int i = 0 ; i < state.length ; i++ ){
            
            state[ i ] = S_BOX[ state[ i ] ];
            
        }
        
        return state;
        
    }
    
    private int[] subBytesReverse( int[] state ){
        
        for ( int i = 0 ; i < state.length ; i++ ){
            
            state[ i ] = S_BOX_REV[ state[ i ] ];
            
        }
        
        return state;
        
    }
    
    private int[] shiftRow( int[] row ){
        
        int placeHolder = row[ 0 ];
        
        for ( int i = 0 ; i < row.length - 1 ; i++ ){
            
            row[ i ] = row[ i + 1 ];
            
        }
        
        row[ row.length - 1 ] = placeHolder;
        
        return row;
        
    }
    
    private int[] shiftRowReverse( int[] row ){
        
        int placeHolder = row[ row.length - 1 ];
        
        for ( int i = 0 ; i < row.length - 1 ; i++ ){
            
            row[ i + 1 ] = row[ i ];
            
        }
        
        row[ 0 ] = placeHolder;
        
        return row;
        
    }
    
    private int[] shiftRowsReverse( int[] state ){
    
        int[][] matrix = toMatrix( state );
        
        for ( int y = 0 ; y < 4 ; y++ ){
            
            int[] row = new int[ 4 ];
            
            for ( int x = 0 ; x < 4 ; x++ ){
                
                row[ x ] = matrix[ x ][ y ];
                
            }
          
            for ( int x = 0 ; x < y ; x++ ){
                
                row = shiftRowReverse( row );
                
            }
            
            for ( int x = 0 ; x < 4 ; x++ ){
                
                matrix[ x ][ y ] = row[ x ];
                
            }
            
        }
        
        for ( int x = 0 ; x < 4 ; x++ ){
            
            for ( int y = 0 ; y < 4 ; y++ ){
            
                state[ x + ( y * 4 ) ] = matrix[ x ][ y ];
                
            }
            
        }
        
        return state;
        
    }
    
    private int[] shiftRows( int[] state ){
    
        int[][] matrix = toMatrix( state );
        
        for ( int y = 0 ; y < 4 ; y++ ){
            
            int[] row = new int[ 4 ];
            
            for ( int x = 0 ; x < 4 ; x++ ){
                
                row[ x ] = matrix[ x ][ y ];
                
            }
          
            for ( int x = 0 ; x < y ; x++ ){
                
                row = shiftRow( row );
                
            }
            
            for ( int x = 0 ; x < 4 ; x++ ){
                
                matrix[ x ][ y ] = row[ x ];
                
            }
            
        }
        
        for ( int x = 0 ; x < 4 ; x++ ){
            
            for ( int y = 0 ; y < 4 ; y++ ){
            
                state[ x + ( y * 4 ) ] = matrix[ x ][ y ];
                
            }
            
        }
        
        return state;
        
    }
    
    private int[] addRoundKey( int[] state , int[] roundKey ){
    
        for ( int i = 0 ; i < state.length ; i++ ){
            
            state[ i ] = state[ i ] ^ roundKey[ i ];
            
        }
        
        return state;
    
    }
    
    private int[] mixColumns( int[] state ){
    
        int[] temp = new int[ 16 ];
      
        for ( int i = 0 ; i < 16 ; i++ ){
            
            int GF = MIX_MATRIX[ i ] - 1;
                
            for ( int x = 0 ; x < 4 ; x++ ){
                
                if ( GF == 0 ){
                    
                    temp[ i ] ^= state[ x ];
                    
                }
                else {
                    
                    temp[ i ] ^= GF_FIELD[ GF ][ state[ x ] ];
                    
                }
               
            }
            
        }
        
        return temp;
    
    }
    
    private int[] mixColumnsReverse( int[] state ){
    
        int[] temp = new int[ 16 ];
        
        for ( int i = 0 ; i < 16 ; i++ ){
            
            int GF = MIX_MATRIX_REVERSE[ i ] - 1;
                
            for ( int x = 0 ; x < 4 ; x++ ){
                
                temp[ i ] ^= GF_FIELD[ GF ][ state[ x ] ];
                
            }
            
        }
        
        return temp;
    
    }
    
    private int[] rotateWord( int[] key ){
        
        key = shiftRow( key );
    
        return key;
        
    }
    
    private int[] subWord( int[] key ){
        
        for ( int i = 0 ; i < key.length ; i++ ){
            
            key[ i ] = S_BOX[ key[ i ] ];
            
        }
        
        return key;
        
    }
    
    private int rCon( int index ){
        
        return R_CON[ index ];
        
    }
    
    private int[] kOffset( int[] key , int offset ){
        
        int[] subKey = new int[ 4 ];
        
        for ( int i = offset ; i < offset + 4 ; i++ ){
        
            subKey[ i - offset ] = key[ i ];
            
        }
         
        return subKey;
        
    }
 
    private int[] keyExpansion( int[] key , int rounds ){
    
        int[] expandedKey = new int[ 16 * ( rounds + 1 ) ];
    
        //Copy first 16 bytes
        for ( int i = 0 ; i < 16 ; i++ ){
            
            expandedKey[ i ] = key[ i ];
            
        }
        
        for ( int i = 4 ; i < 44 ; i++ ){
            
            int[] newKey = new int[ 4 ];
            
            int[] a = kOffset( expandedKey , ( i - 1 ) * 4 );
            int[] b = kOffset( expandedKey , ( i - 4 ) * 4 );
            
            if ( i % 4 == 0 ){
         
                a = rotateWord( a );
                a = subWord( a );
                
                for ( int x = 0 ; x < 4 ; x++ ){
                    
                    newKey[ x ] = a[ x ] ^ b[ x ];
                    
                }
                
                newKey[ 0 ] = newKey[ 0 ] ^ rCon( ( i / 4 ) - 1 );
          
            }
            else {
             
                for ( int x = 0 ; x < 4 ; x++ ){
                    
                    newKey[ x ] = a[ x ] ^ b[ x ]; 
                    
                }
   
            }
            
            for ( int x = 0 ; x < 4 ; x++ ){
                
                expandedKey[ i * 4 ] = newKey[ x ];
                
            }
            
        }
        
        return expandedKey;
        
    }
    
    private int[] encypt( int[] message , int[] key ){
        
        int[] state = message;
        int[] expandedKey = keyExpansion( key , rounds );
        int[] roundKey;
        
        state = addRoundKey( state , key );
        
        for ( int i = 0 ; i < rounds ; i++ ){
            
            roundKey = new int[ 16 ];
            
            for ( int x = 0 ; x < 16 ; x++ ){
                
                roundKey[ x ] = expandedKey[ ( 16 * ( i + 1 ) ) + x ];
                
            }
            
            state = subBytes( state );
            state = shiftRows( state );
            state = mixColumns( state );
            state = addRoundKey( state , roundKey );

        }
        
        roundKey = new int[ 16 ];
            
        for ( int x = 0 ; x < 16 ; x++ ){
                
            roundKey[ x ] = expandedKey[ ( 16 * rounds ) + x ];
                
        }
        
        state = subBytes( state );
        state = shiftRows( state );
        state = addRoundKey( state , key );
        
        return state;
        
    }
    
    private int[] decypt( int[] message , int[] key ){
        
        int[] state = message;
        int[] expandedKey = keyExpansion( key , rounds );
        int[] roundKey;
        
        state = addRoundKey( state , key );
        
        for ( int i = 0 ; i < rounds ; i++ ){
            
            roundKey = new int[ 16 ];
            
            for ( int x = 0 ; x < 16 ; x++ ){
                
                roundKey[ x ] = expandedKey[ ( 16 * ( i + 1 ) ) + x ];
                
            }
            
            state = shiftRowsReverse( state );
            state = subBytesReverse( state );
            state = addRoundKey( state , roundKey );
            state = mixColumnsReverse( state );
            
            
        }
        
        roundKey = new int[ 16 ];
            
        for ( int x = 0 ; x < 16 ; x++ ){
                
            roundKey[ x ] = expandedKey[ ( 16 * rounds ) + x ];
                
        }
        
        state = shiftRowsReverse( state );
        state = subBytesReverse( state );
        state = addRoundKey( state , key );
        
        return state;
        
    }
    
    public String encypt( String message ){
        
        this.message = message;
        
        int[] key = new int[ 16 ];
        String keyString = "ABCDABCDABCDABCD";
        
        for ( int i = 0 ; i < 16 ; i++ ){
            
            key[ i ] = keyString.charAt( i );
            
        }
        
        int[] block = new int[ 16 ];
        
        for ( int i = 0 ; i < 16 ; i++ ){
            
            block[ i ] = message.charAt( i );
            
        }
        
        block = encypt( block , key );
        
        message = "";
        
        for ( int i = 0 ; i < 16 ; i++ ){
            
            message += ( char ) block[ i ];
            
        }
        
        return message;
        
    }
    
    public String decypt( String message ){
        
        int[] key = new int[ 16 ];
        String keyString = "ABCDABCDABCDABCD";
        
        for ( int i = 0 ; i < 16 ; i++ ){
            
            key[ i ] = keyString.charAt( i );
            
        }
        
        int[] block = new int[ 16 ];
        
        for ( int i = 0 ; i < 16 ; i++ ){
            
            block[ i ] = message.charAt( i );
            
        }
        
        block = decypt( block , key );
        
        message = "";
        
        for ( int i = 0 ; i < 16 ; i++ ){
            
            message += ( char ) block[ i ];
            
        }
        
        return message;
        
    }
    
    private static int[] S_BOX = {  };
        
    private static final int[] R_CON = {
    };
    
    private static final int[] GF_2 = { 
        
    };
    
    private static final int[] GF_3 = { 
        
    };

    private static final int[] GF_13 = {
			
					
		};
    
    private static final int[] GF_14 = {
					
					
	};
	
    
    private static final int[] GF_11 = {
		
					
	};
    
    private static final int[] GF_9 = {
			
					
	};
	
    
    private static int[] S_BOX_REV = {  };
    
    private static final int[] MIX_MATRIX_REVERSE = { 14 , 11 , 13 , 9 , 9 , 14 , 11 , 13 , 13 , 9 , 14 , 11 , 11 , 13 , 9 , 14 };
    private static final int[] MIX_MATRIX = { 2 , 3 , 1 , 1 , 1 , 2 , 3 , 1 , 1 , 1 , 2 , 3 , 3 , 1 , 1 , 2 };
    
    private static final int[][] GF_FIELD = { null , GF_2 , GF_3 , null , null , null , null , null , GF_9 , null , GF_11 , null , GF_13 , GF_14 };
    
}
